// POST { blog, seeds:[..], covered:[..titles/keywords..], broaden?:bool }
...
// Returns { clusters[], counts, cost }. Read-only — adding ideas happens client-side.
...
export const handler = async (event) => {
  ...
  if (!dfsConfigured()) return json(500, { error: 'DataForSEO not configured (DATAFORSEO_LOGIN / DATAFORSEO_PASSWORD).' });
  ...
  if (!seeds.length) return json(400, { error: 'provide at least one seed keyword' });
  const covered = (Array.isArray(body.covered) ? body.covered : []).map(s => String(s || '').trim()).filter(Boolean);
  ...
  try {
    ...
    if (body.broaden) calls.push(keywordIdeas(seeds, { limit: 300, minVolume: 30 }));
    ...
    if (!keywords.length) return json(200, { clusters: [], counts: { raw: 0, used: 0 }, cost, note: 'No new keywords found for those seeds (all may already be covered).' });
    ...
    return json(200, { blog: body.blog, seeds, clusters, counts: { raw: rawCount, used: keywords.length, clusters: clusters.length }, cost: Math.round(cost * 10000) / 10000 });
  } catch (e) {
