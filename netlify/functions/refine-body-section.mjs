...
export const handler = async (event) => {
  ...
  const { post_id, instruction, new_html } = body;
  if (!post_id || !instruction || !new_html) return json(400, { error: 'post_id, instruction and new_html required' });
  ...
  try {
    ...
    if (!post || !prop) return json(404, { error: 'no proposed revision for this post' });
    const rev = await refineBodySection({ brand: post.blog, title: post.title || post.primary_keyword, currentBody: new_html, instruction, anthropicKey: AKEY });
    await rest(`body_proposals?id=eq.${prop.id}`, { method: 'PATCH', headers: { ...h, Prefer: 'return=minimal' }, body: JSON.stringify({
      new_html: rev.body_html || new_html, summary: rev.summary || prop.summary, updated_at: new Date().toISOString(),
    }) });
