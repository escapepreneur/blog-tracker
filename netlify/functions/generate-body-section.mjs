...
import { improveBodyAdditive } from './_lib/optimize.mjs';
...
export const handler = async (event) => {
  ...
  try {
    ...
    const gen = await improveBodyAdditive({ brand, title: detail.title, currentHtml: detail.rawHTML, missing: keywords, anthropicKey: AKEY });
    const added_html = gen.added_html || '';
    ...
    await rest(`body_proposals?id=eq.${prop.id}`, { method: 'PATCH', headers: { ...h, Prefer: 'return=minimal' }, body: JSON.stringify({
      added_html, new_html, summary: gen.summary || 'Added a section covering the selected keywords.',
